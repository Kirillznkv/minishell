...
char	**rewrite_env_parse(t_env **env_export, char **new_env)
{
	...
	rewrite_env = (char **)malloc(sizeof(char *) * (len[0] + 1));
	rewrite_env[len[0]] = NULL;
	i = -1;
	ptr = *env_export;
	while (new_env[++i])
	{
		test = check_export_line(ptr, new_env[i]);
		if (test == *env_export)
			rewrite_env[i] = ft_strdup(new_env[i]);
